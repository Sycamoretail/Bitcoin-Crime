...
function get_data(data,father,graph,nodes,depth){
    ...
    if(depth<=0)return;
    let k = 1;
    if(father){  // 如果 father 不为 null，说明现在想要计算的是 father 给当前节点的钱的流向，则忽略其他来源，按照father所占的权重递归计算
        let from_all = 0, from_father = 0;
...
function get_graph(data){
    ...
    data.received_from.forEach(function(d){
        if(d.time<max_time){   // 计算 max_time 以前的总收益作为该节点的收益
